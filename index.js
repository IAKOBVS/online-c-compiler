...
app.post("/compile", (req, res) => {
	...
	if (/[^-\s_+=0-9A-Za-z]/.test(flag)) {
		res.send("Passing illegal characters as compiler flags!<br>Only use characters in [- _+=0-9A-Za-z].");
		return;
	}
	/**
	 * @type {string}
	...
	if (!/\S/.test(compiler)) {
		...
		return;
	} else if (!/^\s*(?:gcc|g\+\+|clang|clang\+\+)-{0,1}[0-9]*\s*$/.test(compiler)) {
	...
	if (!/\S/.test(code)) {
		...
		res.send('Trying to include headers from the filesystem:<br>#include "header.h".<br><br>Use standard includes with angle brackets instead:<br>#include &ltheader.h&gt');
		return;
	}
